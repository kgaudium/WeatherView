...
CURRENT_API_URL = "https://api.gismeteo.net/v2/weather/current/"
...
def _get_headers_by_encoding(encoding_type: Encoding):
...
def request_location_by_ip(ip: str, encoding_type: Encoding = Encoding.NONE, language: Language = Language.RUSSIAN):
    headers = _get_headers_by_encoding(encoding_type)
    return requests.get(f"{LOCATION_API_URL}?lang={language.value}&ip={ip}",
                        headers=headers)
...
def request_location_by_name(query: str, encoding_type: Encoding = Encoding.NONE,
                             language: Language = Language.RUSSIAN):
    headers = _get_headers_by_encoding(encoding_type)
    return requests.get(f"{LOCATION_API_URL}?lang={language.value}&query={query}",
                        headers=headers)
